...
class _RealTimeViewState extends State<RealTimeView> {
        ...
        children: [
              ...
              Material(
                ...
                child: InkWell(
                  ...
                  child: Padding(
                    ...
                    child: Text(l10n.statisticsLabel, style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
                  ),
                ),
              ),
            ],
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              final dividerMargin = ((constraints.maxWidth - 260) * 0.3).clamp(20.0, 90.0);
              return Row(
                ...
                children: [
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.center,
                    children: [
                      Text(l10n.winLossLabel, style: const TextStyle(fontSize: 20, color: Colors.white, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 6),
                      _amountText(_summary?.winLoss, fontSize: 24, color: Colors.white),
                    ],
                  ),
                  Container(
                    width: 1,
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.center,
                    children: [
                      Text(l10n.ngrLabel, style: const TextStyle(fontSize: 20, color: Colors.white, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 6),
                      _amountText(_summary?.ngr, fontSize: 24, color: Colors.white),
                    ],
                  ),
                ],
              );
  ...
  Widget _amountText(int? value, {required double fontSize, required Color color}) {
    ...
    return Text(_fmt.format(value), style: TextStyle(fontSize: fontSize, fontWeight: FontWeight.bold, color: color));
  }
...
class _StatisticsDialogState extends State<_StatisticsDialog> {
  ...
  Widget _headerCell(String text) => Expanded(
        child: Text(text, textAlign: TextAlign.end, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.grey[400])),
      );

  Widget _monthCell(String month) => SizedBox(
        width: _monthColumnWidth,
        child: Text(month, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.white)),
      );
    ...
    return Dialog(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(year, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.bold, color: Colors.white)),
            const SizedBox(height: 18),
            Row(
              children: [
